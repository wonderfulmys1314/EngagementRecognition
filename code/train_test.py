...
video_name = train_label.loc[index, 'ClipID']
name = getName.findall(video_name)[0]
path = csv_path + "/" + name + ".csv"
label_list.append(train_label.loc[index, 'Engagement'])
df = pd.read_csv(path)
...
# 读取凝视类型数据，进行标准化处理
gaze_feature = np.array(df.loc[:299, gaze_columns])
# 读取眼睛数据
eye_feature = np.array(df.loc[:299, eye_columns])
# 读取头部特征
head_feature = np.array(df.loc[:299, head_columns])
# 读取脸部特征
face_feature = np.array(df.loc[:299, face_columns])
# 读取非刚性面部特征
non_ridge_feature = np.array(df.loc[:299, non_ridge_columns])
# 读取强度特征
intense_feature = np.array(df.loc[:299, intense_columns])
# 读取二分类特征
...
X_train = np.concatenate([gaze_feature, eye_feature, head_feature, face_feature, non_ridge_feature, intense_feature], axis=1).reshape(-1, 300, 443)
...
model = load_model("my_model.h5", custom_objects={
    'AttentionLayer3D': AttentionLayer3D,
    'weight_categorical_crossentropy': weight_categorical_crossentropy
})
print(video_name)
print(train_label)
...
# 查看模型注意力机制层的权重
# attention_weights = model.get_layer("attention_layer3d_1").get_weights()


attention_layer = model.get_layer("attention_layer3d_1")
attention_weights = attention_layer.get_weights()
# print(attention_weights)

new_model = Model(model.input, model.get_layer('concatenate_2').output)
weights = new_model.predict(X_train)
# print(weights)
...
# print(attention_weights * weights)
y = np.sum(weights * attention_weights, axis=2)
# print(y)
# print(y.shape)
# y = np.squeeze(y, axis=0)
result = np.squeeze(softmax(y), 0)
x = np.arange(0, 300, 1)
