...
from keras.layers import LSTM, Bidirectional, GRU, TimeDistributed, GlobalAveragePooling1D, GlobalMaxPooling1D, merge
...
# 面部行为单元强度（未标准化）
intense_train = np.load("../original_data/Train/intense_array.npy")
intense_validation = np.load("../original_data/Validation/intense_array.npy")
intense_test = np.load("../original_data/Test/intense_array.npy")

# 融合特征
X_train = np.concatenate([gaze_train, eye_train, head_train, face_train, non_ridge_train, intense_train], axis=2)
del gaze_train, eye_train, head_train, face_train, non_ridge_train, intense_train
X_validation = np.concatenate(
    [gaze_validation, eye_validation, head_validation, face_validation, non_ridge_validation, intense_validation],
    axis=2)
del gaze_validation, eye_validation, head_validation, face_validation, non_ridge_validation, intense_validation
X_test = np.concatenate([gaze_test, eye_test, head_test, face_test, non_ridge_test, intense_test], axis=2)
del gaze_test, eye_test, head_test, face_test, non_ridge_test, intense_test
# X_train = np.concatenate([X_train, X_validation])
# X_train = np.concatenate([X_train, X_test])

# 获取维度
X_all = np.concatenate([X_train, X_validation, X_test])
Y_all = np.concatenate([train_label, validation_label, test_label])
...
#
# print("类比比例")
# print("类别1: ", (len(Y_all[Y_all == 0]) + len(Y_all[Y_all == 1]))/ len(Y_all))
# print("类别3: ", len(Y_all[Y_all == 2]) / len(Y_all))
# print("类别4: ", len(Y_all[Y_all == 3]) / len(Y_all))
#
#
# print("类比比例")
# print("类别1: ", (len(Y_all[Y_all == 0]) + len(Y_all[Y_all == 1])) / len(Y_all))
# print("类别3: ", (len(Y_all[Y_all == 2]) + len(Y_all[Y_all == 3])) / len(Y_all))
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # 学习率最小处对权重进行更新
            ...
            if (epoch - self.SWA_START) % self.update == 0:
                ...
                # for i, weights in enumerate(self.model.layers):
...
class AttentionLayer3D(Layer):
    ...
    def call(self, inputs, mask=None):
        # x = K.permute_dimensions(inputs, (0, 2, 1))
        a = K.softmax(K.tanh(inputs * self.W))
        # a = K.softmax(K.dot(inputs, self.W))
        # a = K.softmax(x * self.W)
        # a = K.permute_dimensions(a, (0, 2, 1))
        outputs = a * inputs
        outputs = K.sum(outputs, axis=1)
        return outputs
...
#
#     # 提取时序局部特征
#     ax = Convolution1D(128, 1, padding="same", use_bias=True, activation="relu")(x)
#     bx = Convolution1D(128, 2, padding="same", use_bias=True, activation="relu")(ax)
#     cx = Convolution1D(128, 3, padding="same", use_bias=True, activation="relu")(bx)
...
#
#     cx = Bidirectional(LSTM(units=32, dropout=0.2, recurrent_dropout=0.2, use_bias=True, return_sequences=True))(x)
#
#     x = Concatenate()([bx, cx])
#
#     x = AttentionLayer()(x)
...
def LSTM_model():
    ...
    x = AttentionLayer3D()(x)
    x = Dropout(0.2)(x)
    x = Dense(32, use_bias=True, activation="relu")(x)
    outputs = Dense(3, use_bias=True, activation="softmax")(x)
    model = Model(inputs, outputs)
    model.summary()
    return model
...
class LossHistory(keras.callbacks.Callback):
    ...
    def loss_plot(self, loss_type, name):
        ...
        if loss_type == 'epoch':
            ...
            result.append(max(self.val_acc[loss_type]))
            print(result)
            # val_loss
...
# 学习率变化
schedule = lambda epoch: LR_schedule(epoch, 0, lr_start=lr_start, lr_end=lr_end, c=c)
...
for train_index, test_index in skf.split(X_all, Y_all):
    ...
    m = Y_all[train_index]
    Y_train = np_utils.to_categorical(Y_all[train_index], 3)
    Y_test = np_utils.to_categorical(Y_all[test_index], 3)
    ...
    swa_result.append(score[1])
    history.loss_plot('epoch', i)
    i += 1

result = pd.DataFrame(result, columns=["accuracy"])
result.to_csv("result.csv")
swa_result = pd.DataFrame(swa_result, columns=["accuracy"])
swa_result.to_csv("swa_result.csv")