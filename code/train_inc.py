...
def LSTM_model():
    ...
    ax = Convolution1D(256, 20, padding="same", use_bias=True, activation="relu")(x)
    ax = BatchNormalization()(ax)
    ax = Convolution1D(256, 10, padding="same", use_bias=True, activation="relu")(ax)
    ax = BatchNormalization()(ax)
    ax = GlobalMaxPooling1D()(ax)

    bx = Convolution1D(256, 20, padding="same", use_bias=True, activation="relu")(x)
    bx = BatchNormalization()(bx)
    bx = Convolution1D(256, 10, padding="same", use_bias=True, activation="relu")(bx)
    bx = BatchNormalization()(bx)
    bx = GlobalMaxPooling1D()(bx)

    cx = Convolution1D(256, 30, padding="same", use_bias=True, activation="relu")(x)
    cx = BatchNormalization()(cx)
    cx = GlobalMaxPooling1D()(cx)

    dx = Convolution1D(256, 30, padding="same", use_bias=True, activation="relu")(x)
    dx = BatchNormalization()(dx)
    dx = GlobalMaxPooling1D()(dx)

    x = Concatenate()([ax, bx, cx, dx])
