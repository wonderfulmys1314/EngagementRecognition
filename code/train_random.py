...
# from load_original_data import load_data
...
# 融合特征
# X_train = np.concatenate([gaze_train, eye_train, itense_train], axis=2)
...
X_validation = np.concatenate(
    ...
    axis=2)
# X_validation = np.concatenate(
#     [gaze_validation, eye_validation, itense_validation],
#     axis=2)
...
# X_test = np.concatenate([gaze_test, eye_test, itense_test], axis=2)
...
Y_all = to_categorical(Y_all, 4)

# X_all = np.concatenate([gaze_train, gaze_validation, gaze_test])
# Y_all = np.concatenate([train_label, validation_label, test_label])
# Y_all = to_categorical(Y_all, 4)
...
class AttentionLayer(Layer):
    ...
    def call(self, inputs, mask=None):
        ...
        a = K.softmax(K.mean(K.tanh(x * self.W), axis=0))
        print(a.shape)
...
class AttentionLayer3D(Layer):
    ...
    def call(self, inputs, mask=None):
        a = K.softmax(K.sum(inputs * self.W, axis=2))
        a = K.reshape(a, [-1, 300, 1])
        outputs = a * inputs
        outputs = K.sum(outputs, axis=1)
        return outputs
...
def LSTM_model():
    ...
    # 提取时序局部特征
    ax = Convolution1D(128, 2, padding="same", activation="relu")(x)
    dx = Convolution1D(128, 2, padding="same", activation="relu")(ax)

    ex = Concatenate()([ax, dx])

    bx = Bidirectional(LSTM(units=32, dropout=0.2, recurrent_dropout=0.2, return_sequences=True))(ex)
    cx = Bidirectional(LSTM(units=32, dropout=0.2, recurrent_dropout=0.2, return_sequences=True))(bx)
    fx = Concatenate()([bx, cx])

    hx = AttentionLayer3D()(fx)
    ix = Dropout(0.2)(hx)
    jx = Dense(32, activation="relu")(ix)
    outputs = Dense(4, activation="softmax")(jx)
...
X_train, X_test, Y_train, Y_test = train_test_split(X_all, Y_all, test_size=0.2, random_state=444)
