...
def LSTM_model():
    ...
    ax = BatchNormalization()(ax)
    ax = Convolution1D(256, 3, padding="same", use_bias=True, activation="relu")(ax)
    ax = BatchNormalization()(ax)
    ax = GlobalMaxPooling1D()(ax)

    bx = Convolution1D(256, 4, padding="same", use_bias=True, activation="relu")(x)
    bx = BatchNormalization()(bx)
    bx = Convolution1D(256, 4, padding="same", use_bias=True, activation="relu")(bx)
    bx = BatchNormalization()(bx)
    bx = GlobalMaxPooling1D()(bx)

    cx = Convolution1D(256, 5, padding="same", use_bias=True, activation="relu")(x)
    cx = BatchNormalization()(cx)
    cx = Convolution1D(256, 5, padding="same", use_bias=True, activation="relu")(cx)
    cx = BatchNormalization()(cx)
    cx = GlobalMaxPooling1D()(cx)

    dx = Convolution1D(256, 6, padding="same", use_bias=True, activation="relu")(x)
    dx = BatchNormalization()(dx)
    dx = Convolution1D(256, 6, padding="same", use_bias=True, activation="relu")(dx)
    dx = BatchNormalization()(dx)
    dx = GlobalMaxPooling1D()(dx)

    x = Concatenate()([ax, bx, cx, dx, ex, fx])

    x = Dropout(0.2)(x)
    x = Dense(256, bias=True, activation="relu")(x)
    x = Dropout(0.2)(x)
