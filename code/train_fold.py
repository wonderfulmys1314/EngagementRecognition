...
def LSTM_model():
    ...
    ax = Convolution1D(256, 3, padding="same",  use_bias=True)(x)
    ax = LeakyReLU(alpha=0.2)(ax)
    ax = AvgPool1D(pool_size=2, strides=1, padding="same")(ax)
    ax = BatchNormalization()(ax)
    
    bx = Convolution1D(256, 5, padding="same",  use_bias=True)(x)
    bx = LeakyReLU(alpha=0.2)(bx)
    bx = AvgPool1D(pool_size=2, strides=1, padding="same")(bx)
    bx = BatchNormalization()(bx)
    
    cx = Convolution1D(256, 7, padding="same",  use_bias=True)(x)
    cx = LeakyReLU(alpha=0.2)(cx)
    cx = AvgPool1D(pool_size=2, strides=1, padding="same")(cx)
    ...
    x = Bidirectional(LSTM(units=64, dropout=0.2, recurrent_dropout=0.2, use_bias=True, return_sequences=True))(x)
    x = BatchNormalization()(x)
    x = Dropout(0.2)(x)
