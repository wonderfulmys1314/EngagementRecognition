...
# 所有标签
train_label = np.load("../scaled_numpy/Train/train_label.npy")
validation_label = np.load("../scaled_numpy/Validation/train_label.npy")
test_label = np.load("../scaled_numpy/Test/train_label.npy")

# 凝视特征
gaze_train = np.load("../scaled_numpy/Train/gaze_array.npy")
gaze_validation = np.load("../scaled_numpy/Validation/gaze_array.npy")
gaze_test = np.load("../scaled_numpy/Test/gaze_array.npy")

# 眼睛坐标
eye_train = np.load("../scaled_numpy/Train/eye_array.npy")
eye_validation = np.load("../scaled_numpy/Validation/eye_array.npy")
eye_test = np.load("../scaled_numpy/Test/eye_array.npy")

# 头部坐标
head_train = np.load("../scaled_numpy/Train/head_array.npy")
head_validation = np.load("../scaled_numpy/Validation/head_array.npy")
head_test = np.load("../scaled_numpy/Test/head_array.npy")

# 面部坐标
face_train = np.load("../scaled_numpy/Train/face_array.npy")
face_validation = np.load("../scaled_numpy/Validation/face_array.npy")
face_test = np.load("../scaled_numpy/Test/face_array.npy")

# 非刚性面部
non_ridge_train = np.load("../scaled_numpy/Train/non_ridge_array.npy")
non_ridge_validation = np.load("../scaled_numpy/Validation/non_ridge_array.npy")
non_ridge_test = np.load("../scaled_numpy/Test/non_ridge_array.npy")

# alphapose人体姿势数据
alphapose_train = np.load("../scaled_numpy/Train/num_array.npy")
alphapose_validation = np.load("../scaled_numpy/Validation/num_array.npy")
alphapose_test = np.load("../scaled_numpy/Test/num_array.npy")

# 面部行为单元强度（未标准化）
itense_train = np.load("../scaled_numpy/Train/intense_array.npy")
itense_validation = np.load("../scaled_numpy/Validation/intense_array.npy")
itense_test = np.load("../scaled_numpy/Test/intense_array.npy")
...
def LSTM_model():
    ...
    # 提取时序局部特征
    ax = Convolution1D(256, 3, padding="same", use_bias=True, activation="relu")(x)
    x = Concatenate()([ax, x])
    ...
    dx = Bidirectional(LSTM(units=32, dropout=0.2, recurrent_dropout=0.2, use_bias=True, return_sequences=True))(bx)
    # ex = Bidirectional(LSTM(units=32, dropout=0.2, recurrent_dropout=0.2, use_bias=True, return_sequences=True))(dx)
